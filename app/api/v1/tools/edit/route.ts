...
import { buildEditingToolPrompt, getImageEditQualityForTool } from "@/lib/prompt-mapping";
...
import { toolEditBodySchema } from "@/lib/validation";
import { ZodError } from "zod";

export async function POST(request: Request) {
  return withProtectedRoute(request, "tools-edit", async () => {
    try {
      const env = getEnv();
      const body = toolEditBodySchema.parse(await request.json());
      ...
      const imageFile = new File([toUint8Array(imageBytes)], `tool-input.${extension}`, { type: body.mimeType });
      const prompt = buildEditingToolPrompt(body.toolId, body.instruction);
...
${JSON.stringify(body.selectedDirection)}` },
      ...
      return jsonError(500, "Tool edit request failed", error instanceof Error ? error.message : error);
    }
  });
}
